...
class AccountAnalyticLine(models.Model):
    ...
    def _running_domain(self):
        ...
        IMPORTANT: user_id must stay in this domain. This is what scopes
        the "Start work / Stop work" button state, and the running lines
        that button_end_work() will stop, to the CURRENT user only.
        Without it, one employee's running timer makes the button show
        as "running" for every user who opens the task, and clicking
        "Stop work" stops every assignee's timer at once, not just the
        clicking user's own.
        """
        return [
            ("date_time", "!=", False),
